...
function Iguana_addcoinLogin(addcoin_data) {
    ...
    if ( addcoin_data.coin == 'KMD' ) {
        ...
        else if ( addcoin_data.mode == '-1' ) {
            logincoinmodeinfo = 'Native';
            var confpath = Shepherd_getConf('komodod')
            console.log(confpath[0].path);
        }
        else { logincoinmodeinfo = 'Basilisk'; }
        var tmpIguanaRPCAuth = 'tmpIgRPCUser@'+sessionStorage.getItem('IguanaRPCAuth');
        
        if ( addcoin_data.mode == '-1' ) {
            /*var setconfig = new Promise(function(resolve, reject) {
                Shepherd_setConf('komodod').done(resolve).fail(reject);
            });
            var startcoin = new Promise(function(resolve, reject) {
                Shepherd_herd('komodod').done(resolve).fail(reject);
            });

            Promise.all([setconfig, startcoin]).then(function() {
                console.log(logincoinfullname + ' config set, and daemon started...')
            });*/
